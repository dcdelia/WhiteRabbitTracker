...
// Shadow call stack log name
std::string m_logCallStackFileName;
// Output file stream object for shadow call stack logging
std::ofstream m_logCallStackFile;
...
/*
Function used to initialize the logger for the shadow call stack
*/
void initLoggerShadowCallStack(std::string fileName) {
	// Check if file name is empty or null
	if (fileName.empty()) {
		fileName = "callstack.log";
	}
	// Save the file name in a global variable
	m_logCallStackFileName = fileName;
	// Create or open the file
	if (m_logCallStackFile.is_open()) {
		return;
	}
	// Otherwise, open the file and return the status
	else {
		m_logCallStackFile.open(m_logCallStackFileName.c_str());
	}
}
...
Function used to initialize the logger for the shadow call stack
*/
void logShadowCallStack(thread_ctx_t* thread_ctx, callStackThreadP callStackThread) {
...
void PIN_FAST_ANALYSIS_CALL update_offending_instruction(thread_ctx_t* thread_ctx, ADDRINT addr) {
...
void instrumentForTaintCheck(INS ins) {
	...
	// Track clock for control transfer instructions (program code only)
	State::globalState* _gs = State::getGlobalState();
	if (itree_search(_gs->dllRangeITree, INS_Address(ins)) == NULL) {
		if (INS_IsDirectBranch(ins)) {
			// clock2 for near things: direct transfers within short distance
			INS_InsertCall(
				ins,
				IPOINT_BEFORE,
				(AFUNPTR)updateClock2,
				IARG_FAST_ANALYSIS_CALL,
				IARG_REG_VALUE, thread_ctx_ptr,
				IARG_END);
		}
		else if (INS_IsBranch(ins) || INS_IsCall(ins) || INS_IsFarCall(ins) ||
				INS_IsFarJump(ins) || INS_IsRet(ins) || INS_IsFarRet(ins)) {
			// clock1 for far things: indirect branches, calls, far calls and jmps
			// shall we be considering ret instructions too? well, the focus changes, so yes
			INS_InsertCall(
				ins,
				IPOINT_BEFORE,
				(AFUNPTR)updateClock1,
				IARG_FAST_ANALYSIS_CALL,
				IARG_REG_VALUE, thread_ctx_ptr,
				IARG_END);
		}
